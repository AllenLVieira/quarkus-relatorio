...
public class CSVUtils {
    ...
    public static ByteArrayInputStream opportunitiesToCSV(List<OpportunityDTO> opportunities) {
        LOGGER.info("Iniciando conversão de oportunidades para CSV.");
        ...
        try (CSVPrinter csvPrinter = new CSVPrinter(new PrintWriter(output), format)) {
            ...
            csvPrinter.flush();
            LOGGER.info("Conversão de oportunidades para CSV concluída com sucesso.");
            return new ByteArrayInputStream(output.toByteArray());
        } catch (IOException e) {
            LOGGER.error("Falha ao importar dados para CSV: {}", e.getMessage());
